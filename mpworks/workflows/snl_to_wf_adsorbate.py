...
from mpworks.firetasks.adsorption_tasks import SetupAdsorptionTasks

__author__ = 'weichen'
__credits__ = 'montoyjh'
...
def snl_to_wf_adsorbate(snl, parameters):
    ...
    spec = {'task_type': 'Setup adsorption tasks', '_priority': priority,
                '_queueadapter': QA_CONTROL}
    fws.append(Firework([SetupAdsorptionTasks()], spec, 
                        name=get_slug(f + '--' + spec['task_type']),fw_id=3))
    connections[2] = [3]

    wf_meta = get_meta_from_structure(snl.structure)
    wf_meta['run_version'] = 'Dec 2015'
